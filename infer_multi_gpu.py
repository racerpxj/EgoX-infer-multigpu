...
def main(args):
    ...
    # ========== Multi-GPU module assignment ==========
    print("Assigning modules to different GPUs...")
    print("Using GPUs: cuda:0(GPU4), cuda:1(GPU5), cuda:2(GPU6), cuda:3(GPU7)")

    # GPU 3: text_encoder + image_encoder (small modules, ~1-2GB)
    pipe.text_encoder = pipe.text_encoder.to("cuda:3")
    pipe.image_encoder = pipe.image_encoder.to("cuda:3")
    print(f"text_encoder device: {next(pipe.text_encoder.parameters()).device}")
    print(f"image_encoder device: {next(pipe.image_encoder.parameters()).device}")

    # GPU 2: vae (used during decode stage)
    pipe.vae = pipe.vae.to("cuda:2")
    print(f"vae device: {next(pipe.vae.parameters()).device}")

    # ========== Transformer split: blocks[:20] on cuda:0, blocks[20:] on cuda:1 ==========
    # First, load transformer to cuda:0, then split blocks
    pipe.transformer = pipe.transformer.to("cuda:0")
    
    # Get number of blocks
    num_blocks = len(pipe.transformer.blocks)
    mid = 16  # Split at 16: 0-15 on cuda:0, 16-39 on cuda:1 (move more to cuda:1 to avoid OOM)
    print(f"Transformer has {num_blocks} blocks, splitting at {mid} (0:{mid} -> cuda:0, {mid}:{num_blocks} -> cuda:1)")
    
    # Move first 16 blocks to cuda:0 (already there)
    # Move remaining blocks to cuda:1
    for block in pipe.transformer.blocks[mid:]:
        block.to("cuda:1")
        # Also move block's internal scale_shift_table to cuda:1
        if hasattr(block, 'scale_shift_table'):
            block.scale_shift_table = block.scale_shift_table.to("cuda:1")
    
    # Move norm_out and proj_out to cuda:1 (output side)
    pipe.transformer.norm_out.to("cuda:1")
    pipe.transformer.proj_out.to("cuda:1")
    # Keep scale_shift_table on cuda:0 (used by first half blocks with temb from condition_embedder)
    # pipe.transformer.scale_shift_table.to("cuda:1")
    
    print(f"transformer.blocks[0:{mid}] device: {next(pipe.transformer.blocks[0].parameters()).device}")
    print(f"transformer.blocks[{mid}:{num_blocks}] device: {next(pipe.transformer.blocks[mid].parameters()).device}")
    
    # norm_out is FP32LayerNorm, check if it has weight
    if hasattr(pipe.transformer.norm_out, 'weight') and pipe.transformer.norm_out.weight is not None:
        print(f"transformer.norm_out device: {pipe.transformer.norm_out.weight.device}")
    else:
        print(f"transformer.norm_out type: {type(pipe.transformer.norm_out)}")
    
    print(f"transformer.proj_out device: {next(pipe.transformer.proj_out.parameters()).device}")
    # ========== Transformer split end ==========

    # scheduler is not nn.Module, skip .to()
    print(f"scheduler type: {type(pipe.scheduler)}")
    print("Multi-GPU module assignment done.")
    # ========== Multi-GPU module assignment end ==========
